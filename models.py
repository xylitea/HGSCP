...
class GMNlayer(MessagePassing):
    ...
    def propagate_match(self, edge_index, size=None, **kwargs):
        ...
        #print(out_attn.size())
        # out_attn = scatter_(self.aggr, out_attn, edge_index[i], dim_size=size[i])  # scatter_版本没有
        torch.scatter(self.aggr, out_attn, edge_index[i], dim_size=size[i], reduce='add')
        #print(out_attn.size())
...
class GCN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        # x = F.relu(x)
        # x = F.dropout(x, training=self.training)
        # x = self.gcnlayer3(x, edge_index)
        # x = F.relu(x)
        # x = F.dropout(x, training=self.training)
        # x = self.gcnlayer4(x, edge_index)
        batch=torch.zeros(x.size(0),dtype=torch.long).to(self.device)
        hg=self.pool(x,batch=batch)
        y  = self.hidden2label(hg)
        return y