...
def getedge_nextsib(node,vocabdict,src,tgt,edgetype):
    ...
    for i in range(len(node.children)):
        ...
        if len(node.children[i].children)>0:
            siblist.append(node.children[i].id)
    for i in range(len(siblist)-1):
        src.append(siblist[i])
        tgt.append(siblist[i+1])
        edgetype.append([1])
        src.append(siblist[i+1])
        tgt.append(siblist[i])
        edgetype.append([edges['Prevsib']])
    for child in node.children:
        getedge_nextsib(child,vocabdict,src,tgt,edgetype)
def getedge_flow(node,vocabdict,src,tgt,edgetype,ifedge=False,whileedge=False,foredge=False,doedge=False):
    token=node.token
    if whileedge==True:
    ...
    if ifedge==True:
        if token=='IfStatement':
            ...
            if len(node.children)==3:
                ...
                edgetype.append([edges['Ifelse']])
    for child in node.children:
        getedge_flow(child,vocabdict,src,tgt,edgetype,ifedge,whileedge,foredge,doedge)
...
def createsimplifytree(root,node,alltokens,node_list,source_code,selected_nodes,parent=None):
    ...
    if token in source_code or token in selected_nodes or parent.token in ["WhileStatement","ForStatement","IfStatement"]:
        id=len(node_list)
...
def createast(project):
    ...
    for index, row in df.iterrows():
        ...
        astdict[order]=programast
        labeldict[order]=row['label']
        order+=1
    sample1=len(astdict)
...
def createseparategraph(astdict,vocablen,vocabdict,device,mode='astonly',simplify=False,nextsib=False,ifedge=False,whileedge=False,foredge=False,doedge=False,blockedge=False,nexttoken=False,nextuse=False):
    pathlist=[]
    treelist=[]
    ...
    for path,newtree in astdict.items():        
        ...
        if mode=='astonly':
            ...
            getedge_flow(newtree,vocabdict,edgesrc,edgetgt,edge_attr,ifedge,whileedge,foredge,doedge)
            if blockedge==True:
