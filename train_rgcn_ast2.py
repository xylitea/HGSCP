...
from createclone_java_orig_rgcn import createast,createggnndata,createseparategraph
import model2
...
parser.add_argument("--doedge", default=False)
...
for project in projects:
    ...
    for count in range(args.counts):
        ...
        for epoch in epochs:# without batching
            ...
            path = 'result/predict_result/RGCN-ast/orig/{}'.format(project)
            if not os.path.exists(path):
                os.mkdir(path)
            testsaves.to_csv(path+'/{}_count_{}_epoch_{}.csv'.format(project, str(count+1), str(epoch+1)))
            
    alltimes.append([parsetime/args.counts,graphbuildtime/args.counts,datasettime/args.counts,traintime/args.counts,testtime/args.counts])
