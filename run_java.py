...
astdict, vocablen, vocabdict, labeldict=createast('ant2')
...
traindata,validdata,testdata=createggnndata(treedict,labeldict,device)
...
for epoch in epochs:# without batching
    ...
    testsaves.to_csv('result/predict_result/{}_epoch_{}'.format(project, str(epoch+1)))
    resfile=open('result/'+args.graphmode+'_epoch_'+str(epoch+1),mode='w')
