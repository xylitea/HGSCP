...
from createclone_java_orig_emb import createast,createggnndata,createseparategraph
import models
...
for project in projects:
    for count in range(args.counts):
        ...
        sample=len(astdict)
        print('sample = ', sample)
        treedict=createseparategraph(astdict, vocablen, vocabdict,id_vector,device,mode=args.graphmode,nextsib=args.nextsib,ifedge=args.ifedge,whileedge=args.whileedge,foredge=args.foredge,blockedge=args.blockedge,nexttoken=args.nexttoken,nextuse=args.nextuse)
        ...
        for epoch in epochs:# without batching
            ...
            path = 'result/predict_result/embed/ast{}'.format(project)
