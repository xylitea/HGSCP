...
from createsimplify_java2 import createast,createggnndata,createseparategraph
import models
...
# projects=['argouml2']
projects=['lucene3'] #'ambari_upsample37' 'ant1 'argouml2', 'hibernate_upsample50', 'jenkins_upsample35', 'jmeter_upsample_42', 'lucene3', 'poi'

for embedding in [150]:
    for project in projects:
        for count in range(4,args.counts):
            astdict, vocablen, vocabdict, labeldict=createast(project)
            sample=len(astdict)
            print('sample = ', sample)
            treedict=createseparategraph(astdict, vocablen, vocabdict,device,mode=args.graphmode,simplify=args.simplify,nextsib=args.nextsib,ifedge=args.ifedge,whileedge=args.whileedge,foredge=args.foredge,blockedge=args.blockedge,nexttoken=args.nexttoken,nextuse=args.nextuse)
            traindata,testdata=createggnndata(treedict,labeldict,device)
            #trainloder=DataLoader(traindata,batch_size=1)
            num_layers=args.num_layers
            model=models.GGNN(vocablen,embedding_dim=embedding,num_layers=num_layers,device=device).to(device)
            ...
            for epoch in epochs:# without batching
                ...
                path = 'result/predict_result/embedding/{}/{}'.format(embedding,project)
